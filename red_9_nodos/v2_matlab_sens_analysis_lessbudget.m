...
min_beta = 1;
max_beta = 40;
sens_coefs = [0.9,0.95,1.05,1.1,1.15,1.2];
...
for ins=1:length(betas)
    for ll=1:length(lams)
        ...
        for sc=1:length(sens_coefs)  
            ...
            while ((~conv) & (counter < 11)) | ((~conv) & (counter >= 11) & (bud > obj_budget))
                ...
                disp(['budget = ', num2str(bud)]);
                if ((abs(bud - obj_budget)/obj_budget) < 0.01) & (bud <= obj_budget)
                    ...
                    beta = (upper+lower)./2;
                    if (counter > 11) & (bud > obj_budget)
                        beta = upper;
                    elseif (counter > 11) & (bud < obj_budget)
                        beta = lower;
                    end
                    disp(['beta = ',num2str(beta)]);
                end
