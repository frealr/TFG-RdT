...
travel_time,alt_time,alt_price,a_nom,tau,sigma,...
a_max,trans_time,s_max,station_hub_cost,distance] = parameters_aero_network();
figure('Position', [100, 100, 650, 170]);
...
%% Figura 5.6 - escenario optimista
...
for bb=1:length(betas)
    ...
    for ll=1:length(lams)
        counter = counter+1;
        lam = lams(ll);
        filename = sprintf('../red_aero/aero_results/prelim_symmetric_logcoef03_optimistic_sol_beta=%d_lam=%d.mat',beta_or,lam);
        load(filename);
...
%% Figura 5.7 - escenario pesimista
...
for bb=1:length(betas)
    ...
    for ll=1:length(lams)
        counter = counter+1;
        lam = lams(ll);
        filename = sprintf('../red_aero/aero_results/prelim_symmetric_logcoef03_pesimistic_sol_beta=%d_lam=%d.mat',beta_or,lam);
        load(filename);
...
figurename = sprintf('./figures5/aero_topologia_pesimista.png');
saveas(gcf, figurename);




%% Figura 5.8 - restriccion de flota
...
for bb=1:length(betas)
    ...
    for ll=1:length(lams)
        counter = counter+1;
        lam = lams(ll);
        filename = sprintf('../red_aero/aero_results/prelim_symmetric_logcoef03_alim_sol_beta=%d_lam=%d.mat',beta_or,lam);
        load(filename);
...
figurename = sprintf('./figures5/aero_topologia_alim.png');
saveas(gcf, figurename);



%% Figura 5.9 - cambio de precios
...
for bb=1:length(betas)
    ...
    for ll=1:length(lams)
        counter = counter+1;
        lam = lams(ll);
        filename = sprintf('../red_aero/aero_results/prelim_symmetric_logcoef03_cprices_sol_beta=%d_lam=%d.mat',beta_or,lam);
        load(filename);
...
figurename = sprintf('./figures5/aero_topologia_cprices.png');
saveas(gcf, figurename);
